...
## Main idea:
The lifecycle manager manages multiple nodes by ensuring that all nodes transition between state sequentially s.t it upholds the specific order give. The order should ensure that nodes with dependencies are ready before the nodes that depend on them. State transitions occur through explicit actions, ie. configure(), activate(), deactivate(), cleanup(), that trigger each node to move from one state to another, forming a finite state machine where each action executes the corresponding transition logic.
...
time_step = 3:  Rover → ●___●___●___●___● → Future  (every 3rd)  
time_step = 10: Rover → ●_______●_______● → Future  (every 10th)  
